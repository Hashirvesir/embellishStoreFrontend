...
const resultMax=6
const Bag = ({res,products}) => {
    const [pageIndex, setPageIndex] = useState(1);
  const { data, error, isLoading } = useSWR(
    `/api/bag-categories?populate=*&pagination[page]=${pageIndex}&pagination[pageSize]=${resultMax}`
    ,
    fetchDataApi,
    {
...
export default Bag
  ...
  export async function getServerSideProps() {
    const res = await fetchDataApi("/api/bag-categories?populate=*")

    const products = await fetchDataApi(
      `/api/bag-categories?populate=*&pagination[page]=1&pagination[pageSize]=${resultMax}`
    );
