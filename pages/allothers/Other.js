...
const resultMax=5
const Other = ({res,products}) => {
    const [pageIndex, setPageIndex] = useState(1);
  const { data, error, isLoading } = useSWR(
    `/api/other-products?populate=*&pagination[page]=${pageIndex}&pagination[pageSize]=${resultMax}`
    ,
    fetchDataApi,
    {
...
export default Other
   ...
   // Data fetch product from strapi api {start}
   export async function getServerSideProps() {
    const res = await fetchDataApi("/api/other-products?populate=*")

    const products = await fetchDataApi(
      `/api/other-products?populate=*&pagination[page]=1&pagination[pageSize]=${resultMax}`
    );
