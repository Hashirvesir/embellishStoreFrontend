...
import BagsOtherRelatedProduct from "@/components/BagsOtherRelatedProduct";
    ...
    <>
      ...
      <ToastContainer/>
        <Wrapper>
          ...
          <BagsOtherRelatedProduct relatedproducts={relatedproducts} />
        </Wrapper>
      </div>
    </>
  );
...
export const getStaticPaths = async () => {
  const products = await fetchDataApi("/api/bag-categories?populate=*");
...
export const getStaticProps = async ({ params: { slug } }) => {
  const product = await fetchDataApi(
    `/api/bag-categories?populate=*&filters[slug][$eq]=${slug}`
  );
  const relatedproducts = await fetchDataApi(
    `/api/bag-categories?populate=*&[filters][slug][$ne]=${slug}`
  );
